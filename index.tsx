...
let shopping: ShoppingItem[] = JSON.parse(localStorage.getItem("shopping") || "[]");
let imageCache: Record<string, string> = JSON.parse(localStorage.getItem("imageCache") || "{}");
...
const searchInput = d.getElementById("searchInput") as HTMLInputElement;
...
const modalPanel = modal.querySelector(".modal-panel")! as HTMLElement;
const recipeTitle = d.getElementById("recipeTitle")!;
const recipeImg = d.getElementById("recipeImg") as HTMLImageElement;
...
const servingsInput = d.getElementById("servingsInput") as HTMLInputElement;
...
const savePdfBtn = d.getElementById("savePdf")! as HTMLButtonElement;
...
async function saveRecipeAsPdf() {
    ...
    const contentToPrint = modalPanel.cloneNode(true) as HTMLElement;
    ...
    if(nutritionInfoEl.style.display !== 'none') {
        const clonedNutrition = contentToPrint.querySelector('#nutrition-info') as HTMLElement;
...
function manageImageCache(title: string, imageUrl: string) {
...
async function generateAndCacheImage(title: string): Promise<string> {
    if (imageCache[title]) {
...
async function generateNutritionInfo() {
    ...
    try {
        ...
        const nutrition: NutritionInfo = JSON.parse(response.text);
...
function parseIngredient(str: string): IngredientParsed {
...
function formatIngredient(ing: IngredientParsed, f: number): string {
...
function changeFont(dir: number){
    ...
    imageObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const img = card.querySelector('img') as HTMLImageElement;
                const imgContainer = card.querySelector('.img-container') as HTMLElement;
