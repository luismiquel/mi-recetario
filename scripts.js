...
const OV = (()=>{/* ... (resto del código sin cambios) ... */
  ...
  const M = new Map([
    ...
    ["patatas bravas", R( ["Patatas","Aceite para freír","Sal","Ajo","Pimentón dulce","Pimentón picante","Tomate triturado","Vinagre"], ["Fríe patatas en dados hasta dorar.","Sofríe ajo; añade pimentón, tomate y sal.","Cocina 10–12 min y ajusta con vinagre.","Sirve patatas con salsa por encima."], "30–40 min" )],
    ["patatas alioli", R( ["Patatas cocidas","Ajo","Huevo o leche (lactonesa)","Aceite suave","Limón o vinagre","Sal","Perejil"], ["Cuece patatas y enfría.","Emulsiona ajo con huevo/leche y aceite, sal y limón.","Mezcla patata en dados con alioli y perejil."], "25–35 min" )],
    ["calamares a la romana", R( ["Calamares","Harina","Huevo","Aceite para freír","Sal","Limón"], ["Seca aros de calamar y sala.","Pasa por harina y huevo batido.","Fríe a 175–180 ºC 1–2 min; escurre.","Sirve con limón."], "20–30 min" )],
    ["gazpacho andaluz", R( ["Tomate","Pepino","Pimiento verde","Ajo","Pan","Aceite de oliva virgen extra","Vinagre","Sal"], ["Tritura con pan hidratado.","Emulsiona con aceite.","Ajusta sal y vinagre, enfría."], "15–20 min + frío" )],
    ["salmorejo cordobés", R( ["Tomate maduro","Pan","Ajo","Aceite de oliva virgen extra","Vinagre","Sal","Jamón","Huevo cocido"], ["Tritura tomate/pan/ajo/sal.","Emulsiona con aceite y vinagre.","Sirve con jamón y huevo."], "15–20 min + frío" )],
    ["fideuá de pescado", R( ["Fideo grueso","Caldo de pescado","Sepia/calamar","Gamba","Ajo","Pimentón","Tomate","Aceite","Sal","Allioli (opcional)"], ["Sofríe sepia y retira.","Ajo+pimentón+tomate.","Tuesta fideo, moja con caldo.","Incorpora sepia/gambas al final; sirve con allioli."], "30–40 min" )],
    ["paella de verduras", R( ["Arroz","Caldo vegetal","Judía verde","Garrafo/garbanzo","Pimiento","Tomate","Ajo","Aceite","Sal","Azafrán"], ["Sofríe verduras.","Añade arroz y nacara.","Moja con caldo y azafrán.","Cocina 18–20 min; reposo 5."], "35–45 min" )],
    ["paella mixta", R( ["Arroz","Caldo","Pollo","Conejo","Gamba/Calamar","Pimiento","Tomate","Ajo","Aceite","Sal","Azafrán"], ["Dora carnes y haz sofrito.","Nacara el arroz y moja con caldo/azafrán.","Cocina 18–20 min, mariscos al final; reposo 5."], "40–55 min" )],
    ["arroz con bogavante", R( ["Arroz","Bogavante","Caldo de marisco","Ajo","Pimentón","Tomate","Aceite","Sal"], ["Marca bogavante.","Sofrito ajo/pimentón/tomate.","Arroz + caldo + bogavante; 18 min."], "35–45 min" )],
    ["tortilla de patatas", R( ["Patatas","Cebolla (opcional)","Huevos","Aceite","Sal"], ["Fríe patata a fuego suave.","Mezcla con huevo y sal.","Cuaja a tu gusto por ambos lados."], "25–35 min" )],
    ["fabada asturiana", R( ["Fabes","Compango","Cebolla","Ajo","Pimentón","Sal"], ["Desala si procede.","Cuece a fuego bajo asustando.","Espuma y rectifica sal."], "120–180 min" )],
    ["marmitako de bonito", R( ["Bonito","Patata","Pimiento","Cebolla","Ajo","Tomate","Caldo","Aceite","Sal"], ["Sofrito y añade patata.","Cubre con caldo.","Bonito al final 2–3 min."], "35–45 min" )],
    ["rabo de toro", R( ["Rabo de toro","Verduras","Vino tinto","Caldo","Aceite","Sal","Harina (ligar)"], ["Dora rabo enharinado.","Sofríe verduras y moja con vino.","Cuece con caldo 2–3 h hasta meloso."], "150–210 min" )],
    ["merluza en salsa verde", R( ["Merluza","Ajo","Perejil","Vino blanco","Caldo","Aceite","Sal","Almejas (opcional)"], ["Ajo + harina ligera.","Vino+caldo+perejil y merluza 5–7 min.","Almejas hasta abrir."], "20–30 min" )],
    ["pulpo a la gallega", R( ["Pulpo cocido","Patata cocida","Aceite","Pimentón","Sal en escamas"], ["Corta pulpo y patata.","Aliña con aceite, pimentón y sal."], "10–15 min" )],
    ["tarta de queso al horno", R( ["Queso crema","Huevos","Azúcar","Nata","Harina (opcional)"], ["Mezcla sin batir de más.","Horno 180 ºC 40–50 min (centro tembloroso).","Enfría y sirve."], "60–90 min con frío" )],
    ["flan casero", R( ["Leche","Huevos","Azúcar","Vainilla","Azúcar para caramelo"], ["Caramelo al molde.","Mezcla leche/huevo/azúcar.","Horno baño maría 160–170 ºC 40–55 min.","Enfría y desmolda."], "70–120 min con frío" )],
    ["arroz con leche", R( ["Arroz","Leche entera","Azúcar","Canela","Limón"], ["Cuece arroz en leche aromatizada, removiendo.","Azúcar al final; enfría."], "45–60 min" )],
    ["brownie sencillo", R( ["Chocolate","Mantequilla","Azúcar","Huevos","Harina","Nueces (opcional)"], ["Funde choco/mantequilla.","Añade huevos y azúcar, luego harina.","Horno 170–180 ºC 20–25 min."], "35–45 min" )]
  ]);
  return { get: (title) => M.get(title.toLowerCase()) || null };
})();

const PASOS_BASE = { /* ... (resto del código sin cambios) ... */
...
function ingredientesPara(cat,t){/* ... (resto del código sin cambios) ... */
  t=t.toLowerCase(); let ing=[]; const ov = OV.get(t); if (ov) return ov.ing;
  if(cat==="Aperitivo"){ if(/empanadill/.test(t)) ing=["Obladas","Atún/carne","Cebolla","Tomate frito","Huevo","Aceite","Sal"]; else if(/alioli/.test(t)) ing=["Patatas","Ajo","Huevo o leche","Aceite suave","Limón/vinagre","Sal","Perejil"]; else if(/boquerones/.test(t)) ing=["Boquerones","Vinagre","Ajo","Perejil","Aceite","Sal"]; else if(/romana|calamar/.test(t)) ing=["Calamares","Harina","Huevo","Aceite","Sal","Limón"]; else if(/hummus/.test(t)) ing=["Garbanzos","Tahini","Ajo","Limón","Aceite","Sal","Comino"]; else ing=["Pan/base","Aceite de oliva","Ajo (opcional)","Sal"];
  } else if(cat==="Primero"){ if(OV.get(t)) ing=OV.get(t).ing; else if(/sopa|caldo/.test(t)) ing=["Caldo","Ajo/Cebolla","Aceite","Sal"]; else if(/crema|vichyssoise|porrusalda|zanahoria/.test(t)) ing=["Verdura principal","Cebolla/Puerro","Caldo","Aceite","Sal"]; else if(/arroz|paella/.test(t)) ing=["Arroz","Caldo","Verduras","Aceite","Sal","Azafrán (opcional)"]; else if(/fideu/.test(t)) ing=["Fideos","Caldo de pescado","Ajo","Pimentón","Tomate","Aceite","Sal"]; else if(/ensalada/.test(t)) ing=["Lechuga/legumbre","Tomate","Cebolla","Aceite","Vinagre","Sal"];
  } else if(cat==="Segundo"){ if(OV.get(t)) ing=OV.get(t).ing; else if(/pollo/.test(t)) ing=["Pollo","Ajo","Aceite","Sal","Pimienta"]; else if(/ternera|solomillo|chulet/.test(t)) ing=["Vacuno","Aceite","Sal","Pimienta"]; else if(/cordero/.test(t)) ing=["Cordero","Ajo","Romero","Aceite","Sal"]; else if(/secreto|lomo|costillas/.test(t)) ing=["Cerdo","Ajo","Pimentón","Aceite","Sal"]; else if(/bacalao|merluza|dorada|lubina|trucha|bonito|atun/.test(t)) ing=["Pescado","Aceite","Ajo/Perejil","Sal"]; else if(/paella|arroz/.test(t)) ing=["Arroz","Caldo","Pollo/Marisco","Pimiento","Tomate","Azafrán","Sal"]; else if(/tortilla|revuelto/.test(t)) ing=["Huevos","Patata/Setas","Aceite","Sal"];
  } else { if(OV.get(t)) ing=OV.get(t).ing; else if(/flan/.test(t)) ing=["Leche","Huevos","Azúcar","Vainilla","Azúcar para caramelo"]; else if(/natilla/.test(t)) ing=["Leche","Yemas","Azúcar","Maicena","Vainilla"]; else if(/arroz con leche/.test(t)) ing=["Arroz","Leche","Azúcar","Canela","Limón"]; else if(/tarta de queso/.test(t)) ing=["Queso crema","Huevos","Azúcar","Nata","Harina (opcional)"]; else if(/bizcocho|galleta|magdalena|tarta/.test(t)) ing=["Harina","Azúcar","Huevos","Mantequilla/Aceite","Levadura"]; else if(/helado|mousse/.test(t)) ing=["Fruta/Yogur/Chocolate","Azúcar","Nata/Leche"]; else ing=["Azúcar","Leche/Huevos/Harina (según postre)"];
  }
  return Array.from(new Set(ing));
}
function tiempo(cat,t){/* ... (resto del código sin cambios) ... */
  const ov=OV.get(t.toLowerCase()); if(ov) return ov.tiempo; t=t.toLowerCase();
...
function build(cat,list){ return list.map(n=>({categoria:cat,titulo:n,imagen:IMG_BY_CAT(cat), ingredientes:ingredientesPara(cat,n), pasos:pasosPara(cat,n), tiempo:tiempo(cat,n)})); }

const RECETAS = [ /* ... (resto del código sin cambios) ... */
  ...build("Aperitivo", APERITIVOS), ...build("Primero", PRIMEROS), ...build("Segundo", SEGUNDOS), ...build("Postre", POSTRES)
];
...
const els = { /* ... (resto del código sin cambios) ... */
...
function setActiveTab(btn){/* ... (resto del código sin cambios) ... */
...
function photoFallback(title, categoria){
  // Usamos el título para generar un número único (hash)
  let h=0; for(let i=0;i<(title||'').length;i++){ h=((h<<5)-h)+title.charCodeAt(i); h|=0; }
  const lock = Math.abs(h);

  // Usamos 'picsum.photos' con una "seed" para que cada receta tenga siempre la misma imagen
  // Esto es mucho más fiable y no da errores de CORS.
  return `https://picsum.photos/seed/${lock}/800/500`;
}
// ==================================================================

function svgPlaceholder(title="Receta"){/* ... (resto del código sin cambios) ... */
...
async function fetchJSON(url, ms=6000){/* ... (resto del código sin cambios) ... */
  const c = new AbortController(); const t = setTimeout(()=>c.abort(), ms);
  try{ const r = await fetch(url, {signal:c.signal}); if(!r.ok) return null; return await r.json(); }
  catch{ return null; } finally{ clearTimeout(t); }
}
async function wikiSearchTitle(q, lang="es"){/* ... (resto del código sin cambios) ... */
...
async function wikiThumbForTitle(title, lang="es"){/* ... (resto del código sin cambios) ... */
...
async function commonsThumb(query){/* ... (resto del código sin cambios) ... */
  ...
  const j = await fetchJSON(url); const pages = j?.query?.pages; if(!pages) return null; const first = Object.values(pages)[0];
  return first?.imageinfo?.[0]?.thumburl || first?.imageinfo?.[0]?.url || null;
}
async function fetchDishImageURL(dish){/* ... (resto del código sin cambios) ... */
...
async function ensureDishBlob(title){/* ... (resto del código sin cambios) ... */
  const key = slug(title); const cached = await idbGet(key);
...
async function objectURLFor(title, categoria){/* ... (resto del código sin cambios) ... */
  try{ const b = await ensureDishBlob(title); if(b) return URL.createObjectURL(b); }catch{}
  return photoFallback(title, categoria);
}
async function setImg(el, title, categoria){/* ... (resto del código sin cambios) ... */
  try{
    ...
    el.onerror = async ()=>{
      ...
      try{ const r = await fetch(bust, {cache:'no-store'}); if(r.ok){ el.src = bust; return; } }catch{}
...
// El resto de tu código JavaScript sigue aquí sin cambios...
function card(r,i){/* ... */
  const star = isFav(r) ? '⭐' : '☆';
  const ph = svgPlaceholder(r.titulo);
  return `<article class="card" role="article" aria-label="${r.titulo}"><img src="${ph}" data-dish="${r.titulo}" data-cat="${r.categoria}" alt="Foto de ${r.titulo}" class="skeleton"><div class="body"><h3 style="margin:.25rem 0;font-size:1.05rem">${r.titulo}</h3><p class="meta">${r.categoria} · ${r.tiempo}</p><div class="bar"><button class="btn btn-ghost" data-a="leer" data-i="${i}" aria-label="Leer ${r.titulo} en voz alta" title="Leer">🔈 Leer</button><button class="btn btn-primary" data-a="guiada" data-i="${i}" aria-label="Cocina guiada para ${r.titulo}" title="Guiada">▶️ Guiada</button><button class="btn btn-ghost" data-a="abrir" data-i="${i}" aria-label="Abrir detalles de ${r.titulo}" title="Abrir">📖 Abrir</button><button class="btn btn-ghost" data-a="lista" data-i="${i}" aria-label="Añadir ingredientes de ${r.titulo} a la lista" title="Añadir a lista">🧺 Añadir</button><button class="btn btn-ghost" data-a="fav" data-i="${i}" aria-label="Marcar favorito" title="Favorito">${star} Favorito</button></div></div></article>`;
}
function render(list){/* ... */
  state.list = list;
  if(!list.length){ els.grid.innerHTML = ""; els.empty.hidden = false; els.count.textContent = "Mostrando 0 recetas."; return; }
  els.empty.hidden = true; els.grid.innerHTML = list.map((r,i)=>card(r,i)).join(""); els.count.textContent = `Mostrando ${list.length} recetas.`;
  try{ injectListLD(list); }catch{}
  requestIdleCallback?.(()=> { document.querySelectorAll('img[data-dish]').forEach(img=>{ setImg(img, img.dataset.dish, img.dataset.cat); }); }) || document.querySelectorAll('img[data-dish]').forEach(img=>setImg(img, img.dataset.dish, img.dataset.cat));
}
function filter(){/* ... */
  const q = state.q.toLowerCase().trim();
  const lAll = RECETAS.filter(r => (state.cat==="Todas" || r.categoria===state.cat) && (!q || [r.titulo, ...r.ingredientes, ...r.pasos].join(" ").toLowerCase().includes(q)));
...
function openDetail(r){/* ... */
  state.current = r;
  els.detailTitle.textContent = r.titulo; els.detailImg.src = svgPlaceholder(r.titulo); setImg(els.detailImg, r.titulo, r.categoria);
  els.detailMeta.textContent = `${r.categoria} · ${r.tiempo}`; els.detailIngredients.innerHTML = r.ingredientes.map(i=>`<li>${i}</li>`).join("");
  els.detailSteps.innerHTML = r.pasos.map(p=>`<li>${p}</li>`).join(""); els.detailModal.style.display='flex';
  injectRecipeLD(r);
...
function pickVoice(){/* ... */
  const vs = speechSynthesis.getVoices(); voicesReady = vs && vs.length>0;
  ttsVoice = vs.find(v=>/Spanish.*(Spain)|es[-_]ES/i.test(v.name+v.lang)) || vs.find(v=>/Spanish|es[-_]/i.test(v.name+v.lang)) || vs[0] || null;
}
...
function unlockTTSOnce(){/* ... */
  ...
  document.removeEventListener('touchend', unlockTTSOnce); document.removeEventListener('click', unlockTTSOnce);
}
document.addEventListener('touchend', unlockTTSOnce, {passive:true}); document.addEventListener('click', unlockTTSOnce, {once:true});
let speakLock=false;
...
async function speakSafe(text){/* ... */
  ...
  u.lang = (ttsVoice && ttsVoice.lang) || 'es-ES'; u.rate = state.rate; u.pitch = 1.0;
...
function ensureCurrent(){/* ... */
  if(!state.current){ const primera = (state.list && state.list[0]) || null; if(!primera){ els.status.textContent="Abre una receta y pulsa ‘Guiada’."; return false; } state.current = primera; state.step = 0; }
  return true;
}
...
function readCurrent(){/* ... */
  ...
  clearTimerUI();
  try{ const p = state.current?.pasos?.[state.step] || ""; const m = /(\d{1,2})\s*(?:min|mins|minutos)\b/i.exec(p); if(m){ const mins = Number(m[1]); if(mins>0){ speakSafe(`Inicio temporizador de ${mins} minutos.`); startStepTimer(mins); } } }catch{}
}
...
function renderList(){/* ... */
  $('shoppingList').innerHTML = shopping.map(i=>`<li class="row" style="align-items:center"><input type="checkbox" ${i.done?'checked':''} data-a="t" data-id="${i.id}" aria-label="Marcar ${i.text}" title="Marcar ${i.text}"><input type="text" value="${i.text.replace(/"/g,'&quot;')}" style="flex:1;background:transparent;border:0;border-bottom:1px solid #d1d5db;padding:.25rem .35rem" data-a="e" data-id="${i.id}" aria-label="Editar ${i.text}" title="Editar ${i.text}"><button class="btn btn-ghost" data-a="d" data-id="${i.id}" aria-label="Eliminar ${i.text}" title="Eliminar ${i.text}">🗑️</button></li>`).join("");
...
function beep(ms=200, freq=880){/* ... */
  try{ const ctx = new (window.AudioContext||window.webkitAudioContext)(); const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.connect(gain); gain.connect(ctx.destination); osc.frequency.value = freq; osc.start(); setTimeout(()=>{ osc.stop(); ctx.close(); }, ms); }catch{}
}
function startStepTimer(minutes){/* ... */
  clearTimerUI();
  if(currentTimer) { clearTimeout(currentTimer); currentTimer = null; }
  const box = document.createElement('div');
  box.id='guidedTimer'; box.className='meta'; box.style.marginTop='.5rem';
  box.innerHTML = `<span>⏱️ Temporizador: <strong id="tleft"></strong></span> <button class="btn btn-ghost" id="cancelTimer" aria-label="Cancelar temporizador" title="Cancelar temporizador" style="margin-left:.5rem">Cancelar</button>`;
...
function injectRecipeLD(r){/* ... */
  const ld = { "@context":"https://schema.org", "@type":"Recipe", "name": r.titulo, "recipeCategory": r.categoria, "image": " ", "recipeIngredient": r.ingredientes, "recipeInstructions": (r.pasos||[]).map(p=>({"@type":"HowToStep","text":p})), "totalTime": "PT30M" };
...
function saveSession(){/* ... */
  try{ if(!state.current) return; localStorage.setItem(SKEY_SESSION, JSON.stringify({ slug: slug(state.current.titulo), step: state.step||0 })); }catch{}
}
function loadSession(){/* ... */
  try{
    const raw = localStorage.getItem(SKEY_SESSION); if(!raw) return null;
    const d = JSON.parse(raw); const r = RECETAS.find(x => slug(x.titulo) === d.slug);
...
const SRClass = (!isIOS) && (window.SpeechRecognition||window.webkitSpeechRecognition) ? (window.SpeechRecognition||window.webkitSpeechRecognition) : null;
let recAuto = null;
...
function makeRecognizer(){/* ... */
  ...
  const r=new SRClass(); r.lang='es-ES'; r.continuous=true; r.interimResults=true; r.maxAlternatives=4;
  return r;
}
function startAutoRecognizer(){/* ... */
  if(!SRClass){ setStatus(isIOS ? "Voz: iPhone/iPad no permite reconocimiento web. Usa botones." : "Voz: reconocimiento no disponible en este navegador."); document.getElementById('voiceHUD')?.classList.add('disabled'); return; }
...
document.addEventListener('DOMContentLoaded', ()=>{ filter(); renderList(); });
function renderList(){}
document.getElementById('detailModal').addEventListener('click', e => { if(e.target.id==='detailModal') closeDetail(); });
